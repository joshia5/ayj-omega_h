...
void coarsen_curved_verts_and_edges(Mesh *mesh, Mesh *new_mesh, const LOs old2new,
    const LOs prods2new, const LOs old_verts2new_verts, const LOs keys2verts,
    const LOs keys2verts_onto, const LOs keys2prods, const LOs 
    same_verts2new_verts, const LOs same_verts2old_verts) {
  ...
  auto curve_bdry_edges = OMEGA_H_LAMBDA(LO i) {
	...
	edge_crv2bdry_dim[new_edge] = 2;
          ...
          for (LO cand = 0; cand < nedge_shared_gface_i; ++cand) {
            ...
            if (std::abs(upper_theta - PI) < 1e-6) {
              ...
              */
              ...
              if ((std::abs(n[1])<1e-6) && ((std::abs(n[0])-1.0)<1e-6)
                  && (std::abs(n[2])<1e-6)) {
                printf("x axis\n");
              ...
              if ((std::abs(n[0])<1e-6) && ((std::abs(n[2])-1.0)<1e-6)
                  && (std::abs(n[1])<1e-6)) {
                printf("z axis\n");
                //made by straight edge with uppere0
                X[0] = upper_tangents[0]*cos(theta_c) -
            ...
            printf("new edge %d tangent{%f,%f,%f} n{%f,%f,%f}\n",new_edge,
        ...
        if (std::abs(length_t - 0.0) < 1e-6) {
          ...
          auto X = A_inv*b;
          //hand-calc +/- x axis as tang if A is ill-cond
          if (std::abs(lower_theta - PI) < 1e-6) {
            ...
            //+x
            if (((std::abs(n[0])-1.0)<1e-6) && (std::abs(n[1])<1e-6)
                && (std::abs(n[2])<1e-6)) {
