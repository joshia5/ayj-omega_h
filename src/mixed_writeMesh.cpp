...
void twotri_square_gen(CommPtr comm, std::string const mesh_dir) {
  auto mesh = Mesh(comm->library());
  try {
    int numVerts = 4;
    ...
    int numElems = 2;
    int elementType[2] = {5, 5};
    int elementData[6] = {0,1,2, 2,1,3};
    pVertex vReturn[4];
    pEntity eReturn[2];
    std::string mesh_path = mesh_dir + "/Example_2tri_square.sms";
