...
void test_2d(Library *lib, const std::string &mesh_file, const char* vtu_file) {
  ...
  // edge 1 (diameter, starts at pi/4, on for pi)

  {
    ...
    for (LO i = 0; i < u.size(); ++i) {
      auto x_bezier = p0(0,0)*B0(u[i]) + Cx(0,0)*B1(u[i]) + Cx(1,0)*B2(u[i]) + p1(0,0)*B3(u[i]);
      auto y_bezier = p0(1,0)*B0(u[i]) + Cy(0,0)*B1(u[i]) + Cy(1,0)*B2(u[i]) + p1(1,0)*B3(u[i]);
  ...
  // edge 2 (along x, starts at 5*pi/4 goes on for pi/2)
  {
    Real s_c1 = PI/2 * xi_1;
    Real s_c2 = PI/2 * xi_2;
    Matrix<2,1> p0({1, 0});//ask coords
    Matrix<2,1> p1({0, 0});
  ...
  // edge 3 (along y, starts at 7*pi/4 goes on for pi/2)
  {
    Real s_c1 = PI/2 * xi_1;
    Real s_c2 = PI/2 * xi_2;
    Matrix<2,1> p0({0, 0});//ask coords
    Matrix<2,1> p1({0, 1});
